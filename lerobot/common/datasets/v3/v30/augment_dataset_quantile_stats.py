...
def process_single_episode(dataset: LeRobotDatasetV3, episode_idx: int) -> dict:
...
def compute_quantile_stats_for_dataset(dataset: LeRobotDatasetV3) -> dict[str, dict]:
...
def augment_dataset_with_quantile_stats(
    ...
    try:
        ...
        logging.info(f"tag={CODEBASE_VERSION_V3} probably doesn't exist. Skipping exception ({e})")
        pass
    hub_api.create_tag(repo_id, tag=CODEBASE_VERSION_V3, revision=None, repo_type="dataset")
