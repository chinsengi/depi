...
class PaliGemmaWithExpertModel(PreTrainedModel):
    ...
    def get_attention_interface(self):
        if self.config.attention_implementation == "fa2":
            ...
            # flex attention is slower than eager attention
            attention_interface = self.flex_attention_forward
        else:
            # attention_interface = self.torch_eager_attention_forward
    ...
    def flash_attention_forward(
        ...
        raise NotImplementedError(
            "FA2 is not implemented, because it does not support custom attention mask for now."
        )
    ...
    def torch_eager_attention_forward(
        self,
        attention_mask,
        batch_size,
        head_dim,
        ...
        # Convert attention mask to proper format for SDPA
        # attention_mask: [batch_size, seq_len] -> need to convert to [batch_size, 1, seq_len, seq_len]
        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(1)  # [batch_size, 1, seq_len]

        # Transpose to [batch_size, num_heads, seq_len, head_dim] for SDPA
        query_states = query_states.transpose(1, 2)
        key_states = key_states.transpose(1, 2)
        ...
        att_output = torch.nn.functional.scaled_dot_product_attention(
            ...
            is_causal=False,  # We handle causal masking manually
        )

        # Transpose back and reshape to match expected output format
        att_output = att_output.transpose(1, 2).reshape(batch_size, sequence_length, num_att_heads * head_dim)
    ...
    def flex_attention_forward(
        ...
        # Pad query, key and value states to match the rounded lengths
