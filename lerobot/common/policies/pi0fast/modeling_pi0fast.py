...
class PI0FASTPolicy(PreTrainedPolicy):
    ...
    def _pi_aloha_encode_actions_inv(self, actions):
        # ALOHA (A LOw-cost Hand for Automation) is a robotics system that uses two WidowX robot arms
        # This function inverts the encoding of actions for ALOHA compatibility
        # Flip specific joint angles back to ALOHA's coordinate system
        for motor_idx in [1, 2, 8, 9]:
            actions[:, :, motor_idx] *= -1
        # Reverse the gripper transformation that is being applied by the Aloha runtime.
        for motor_idx in [6, 13]:
...
class PI0FAST(nn.Module):
    ...
    def generate_actions(self, batch: dict[str, Tensor]):
        ...
        padded_outs = self.create_input_tokens(
            state=batch[OBS_ROBOT], lang_text=batch["task"], actions=None
        )
