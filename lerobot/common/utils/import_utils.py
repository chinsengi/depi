...
def make_device_from_device_class(config: ChoiceRegistry) -> Any:
    ...
    device_class_name = config_name[:-6]  # MyDeviceConfig -> MyDevice

    # Build candidate modules to search for the device class
    parts = module_path.split(".")
    parent_module = ".".join(parts[:-1]) if len(parts) > 1 else module_path
    candidates = [
        parent_module,  # lerobot_teleop_mydevice
        parent_module + "." + device_class_name.lower(),  # lerobot_teleop_mydevice.mydevice
    ]
